...
export const parse = async (
  ...
  printer.slides = await markedParse(content).then((html) => (
  ...
  if (stylesheet) {
    printer.styles.push(stylesheet);
  }

  return printer;
};

const markedParse = async (src: string) => marked.parse(src, {
  async: true,
